# https://www.ursinaengine.org/tic_tac_toe.html
# потрібно зробити щоб перезапускалась гра
from ursina import *
...
def check_for_victory():
    ...
    if won:
        ...
        # randomize_button = Button(position=(0,-.2), origin=(0,0), color=color.dark_gray, text='<white>re Start', scale=(.25, .05))
        randomize_button = Button(position=(0,-.2), origin=(0,0), color=player.color.tint(-.1), text='<white>start', scale=(.25, .05))
        randomize_button.scale *= 1.75
        # randomize_button.on_click = on_mouse_exit()
